...
do
	...
	do
		echo "**** $i, iteration: $j ****"
		largeFileName="8GB_child"	#No separate parent/child needed in Ext4. We can use single file only and treat it as child.
		...
		#Run fio
		fioOut="${fioOutName}${i}_${fs}_prealloc_${dev}"
	...
	: '
        ...
        if [ $? != 0 ]
        then
                echo "Error: $0 failed to save mem util file into '$outPath'."
		exit -1
        fi
	'
