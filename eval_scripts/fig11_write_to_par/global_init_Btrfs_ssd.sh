...
do
	...
	cp $i $dest 
        ...
        for p in "${numChildren[@]}"
        do
		echo "*************** $p children ***************"
                fioOut=""
		for ((j=0; j<end; j++))
		do
			echo "**** $i, $p children, iteration: $j ****"
			...
			#fstrim -v .
                        ...
                        for((q=1;q<=$p;q++))
                        do
                                childFile="${largeFileName}_child${q}"
                                frndFile="${largeFileName}_frnd${q}"
                                echo "childFile: $childFile"
                                echo "parentFile: $largeFileName"

				cp --reflink $largeFileName $childFile
                        done
			...
			#Run fio
			fioOut="${fioOutName}${i}_${fs}_${dev}_${p}kids"
