...
set $meandirwidth=20
set $nfiles=32768
set $filesize=cvar(type=cvar-gamma,parameters=mean:262144;gamma:1.5)
#set $nfiles=10000
...
#########################################################################
#Rohit: Note that parent directory name is bigfileset
#       Child directory name is bigfileset_child 
#       (child name is controlled by our auto.sh script)
#       We want this workload to operate on child files
#       So, changed name of fileset in this file to bigfileset_child
#       flowop operations are also happening on bigfileset_child fileset
#########################################################################
define fileset name=bigfileset,path=$dir,size=$filesize,entries=$nfiles,dirwidth=$meandirwidth,prealloc,reuse,trusttree,readonly
