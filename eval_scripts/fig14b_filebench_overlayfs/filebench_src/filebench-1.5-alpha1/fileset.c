...
fileset_alloc_file(filesetentry_t *entry)
{
	...
	//filebench_log(LOG_INFO, "Populated %s", entry->fse_path);

	/* see if reusing and this file exists */
	trust_tree = avd_get_bool(fileset->fs_trust_tree);
	if ((entry->fse_flags & FSE_REUSING) && (trust_tree || (FB_STAT(path, &sb) == 0))) {
		/* Rohit */
		//filebench_log(LOG_INFO, "Will Re-use file %s", path);
		/* Rohit: This open has to be in RD mode only. Otherwise,
		 * copy up gets triggered in overlayfs
		 */
		...
		/* Rohit */
		//filebench_log(LOG_INFO, "Won't Re-use file %s. Creating it.", path);
...
fileset_create(fileset_t *fileset)
{
	...
	while ((entry = fileset_pick(fileset, FILESET_PICKFREE | FILESET_PICKFILE, 0, 0))) {
		pthread_t tid;
		int newrand;

		/*Rohit*/
		//filebench_log(LOG_INFO, "inside while loop that loops over fileset_pick()");

		newrand = rand();

		/* Rohit: Commented out this check for 'reuse' command to work correctly
		 * However, observed that if prealloc is set to 100, then also this if condition won't run
		 * So, uncommented it.
		 *
		 */
		if (randno && newrand <= randno) {
			// unbusy the unallocated entry  //
			fileset_unbusy(entry, TRUE, FALSE, 0);
			//filebench_log(LOG_INFO, "inside while loop: randno issue: randno: %d, newrand: %d", randno, newrand);
			continue;
		}

		preallocated++;
		/*Rohit*/
		//filebench_log(LOG_INFO, "inside while loop: preallocated: %d", preallocated);
		...
		if (avd_get_bool(fileset->fs_paralloc)) {
			/*Rohit*/
			//filebench_log(LOG_INFO, "inside while loop: paralloc is set");
...
fileset_populate_file(fileset_t *fileset, filesetentry_t *parent, int serial)
{
	...
	(void) ipc_mutex_unlock(&fileset->fs_pick_lock);
	/* Rohit */
	//filebench_log(LOG_INFO, "fileset_populate_file called");
...
fileset_populate_leafdir(fileset_t *fileset, filesetentry_t *parent, int serial)
{
	...
	/* Rohit */
	//filebench_log(LOG_INFO, "fileset_populate_leafdir called");
    ...
    int serial, double depth)
{
	...
	for (i = 1; ((parent == NULL) || (i < ranwidth + 1)) &&
	    ...
	    i++) {
		/* Rohit */
		//filebench_log(LOG_INFO, "fileset_populate_subdir: loop 1");
		int ret = 0;
	...
	for (i = 1; ((parent == NULL) || (i < ranwidth + 1)) &&
		...
		int ret = 0;
		/* Rohit */
		//filebench_log(LOG_ERROR, "fileset_populate_subdir: loop 2");
	...
	/* Rohit */
	//filebench_log(LOG_INFO, "fileset_populate_subdir: loops ended. Returning.");
...
fileset_populate(fileset_t *fileset)
{
	...
	avl_create(&(fileset->fs_dirs), fileset_entry_compare,
	    sizeof (filesetentry_t), FSE_OFFSETOF(fse_link));
	/* Rohit */
	//filebench_log(LOG_INFO, "Initialized avl btrees");
