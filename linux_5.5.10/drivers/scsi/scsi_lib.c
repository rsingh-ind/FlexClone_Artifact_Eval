...
void scsi_io_completion(struct scsi_cmnd *cmd, unsigned int good_bytes)
{
	...
	//scorw start
	void *kaddr_r = 0;
        kaddr_r = kmap_atomic(bio_page(req->bio));
	printk("[pid: %u]scsi_io_completion: kaddr_r: %c%c%c%c, offset: %d\n", current->pid, *((char*)kaddr_r + 0), *((char*) kaddr_r + 1), *((char*)kaddr_r + 2), *((char*)kaddr_r + 3), bio_offset(req->bio));
