...
//Create new handle for journal write
//If handle already exists, simply return that handle
handle_t *jbd2__journal_start(journal_t *journal, int nblocks, int rsv_blocks,
			      int revoke_records, gfp_t gfp_mask,
			      unsigned int type, unsigned int line_no)
{
	//obtain handle corresponding current task
	//can be null or already present
	...
	if (handle) {
		//cross check that handle we found here belongs to same
		//journal which is passed as argument to this function 
		J_ASSERT(handle->h_transaction->t_journal == journal);

		//increase reference count
		handle->h_ref++;

		//return handle if already exists
		return handle;
	}

	//maybe, number of blocks required for this transaction
	//
	nblocks += DIV_ROUND_UP(revoke_records,
				journal->j_revoke_records_per_block);

	//allocate new handle with
	//total credits equal to nblocks and ref count equal to 1
	//
	handle = new_handle(nblocks);
	if (!handle)
		return ERR_PTR(-ENOMEM);

	//reserved blocks handle
	//What are these reserved blocks?
	if (rsv_blocks) {
		handle_t *rsv_handle;
...
{
	//printk("Inside jbd2_journal_file_inode\n");
	transaction_t *transaction = handle->h_transaction;
	journal_t *journal;
...
{
	//printk("Inside jbd2_journal_inode_ranged_wait\n");
